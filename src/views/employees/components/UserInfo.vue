...
export default {
  ...
  methods: {
    async saveUser() {
      const list = this.$refs.staffPhoto.fileList
      if (list.some((e) => !e.upload)) {
        this.$message.warning('此时还有图片未上传成功！')
        return
      }
      /* 第一部分保存 */
      await saveUserInfo({ ...this.userInfo, staffPhoto: list.length ? list[0].url : ' ' })
      this.$message.success('保存用户基本信息成功！')
    },
    async savePersonal() {
      const list = this.$refs.myStaffPhoto.fileList
      if (list.some((e) => !e.upload)) {
        this.$message.warning('此时还有图片未上传成功！')
        return
      }
      /* 第二部分保存 */
      await UpdateUserDetailById({ ...this.formData, staffPhoto: list.length ? list[0].url : ' ' })
